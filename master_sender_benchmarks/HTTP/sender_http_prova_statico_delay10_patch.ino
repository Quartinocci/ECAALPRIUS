...
// if checking on laptop teh traffic "http://192.168.1.205:8080/data";
const char* serverUrl = "http://192.168.1.201/data";
...
void run_or_benchmark_change(float extra_cooldown_time_between_runs)
{
    ...
    //bit of dilemma here cause we might read (due to INA226 way of working in continous mode) soem data unrelated to task we want to measure 
    //shouldnt be too much of a problem since we dont miss any data thanks to the samples representing averages. plus even if we were sampling in the classsic manner the unrelated data would be such a small percentage that would nt represnet a problem.
}
